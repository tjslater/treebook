...
test "that creating a friendship works without raising an exception" do
 ...
 context "a new instance" do
  	setup do
  		@user_friendship = UserFriendship.new user: users(:baggins), friend: users(:samwise)
  	end
 ...
 context "#send_request_email" do
  	setup do
  		@user_friendship = UserFriendship.create user: users(:baggins), friend: users(:samwise)
  	end
  ...
  context "#accept!" do
  	setup do
  		@user_friendship = UserFriendship.request users(:baggins), users(:samwise)
  	end
 	...
 	context ".request" do
		should "create two user friendships" do
			assert_difference 'UserFriendship.count', 2 do
				UserFriendship.request(users(:baggins), users(:samwise))
			end
		end
  ...
  context "#block!" do
    setup do
      @user_friendship = UserFriendship.request users(:baggins), users(:gandalf)
  ...
  should "not allow new requests once blocked" do
    @user_friendship.block!
    uf = UserFriendship.request users(:baggins), users(:gandalf)
    assert !uf.save
  end
end
